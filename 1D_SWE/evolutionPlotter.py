...
network_dataPINN = load_model('trained_networks/SWE_maxiter10000_b'+str(benchmark)+'_dataAndPhysics.h5')
...
if plot == 1:
    ...
    ModelInfo = InputVariables(tend = 1)
    ...
    plt.plot(T, h_pinns, label = 'h PINNs', color = 'blue')
    ...
    plt.plot(ModelInfo.t_num, ModelInfo.h_num[qua, :], label = 'h numerical', color = 'red')
    plt.xlabel('Time (s)')
    plt.ylabel('Height (m)')
    ...
    plt.plot(T, uh_pinns, label = 'uh PINNs', color = 'blue')
    ...
    plt.plot(ModelInfo.t_num, ModelInfo.uh_num[qua, :], label = 'uh numerical', color = 'red')
    plt.xlabel('Time (s)')
...
if plot == 2:
    ModelInfo = InputVariables(tend = 0.4)
    ...
    T = np.round(np.linspace(0, len(t_num) - 1, 6)).astype(int)
    ...
    for i, t_cs in enumerate(t_cross_sections):
        ...
        plt.ylim(minH, maxH)
        plt.plot(x_num, h_pinn, label = 'h PINNs', color = 'blue')
        ...
        plt.plot(x_num, h_num[:, T[i]], label = 'h numerical', color='red')
    ...
    for i, t_cs in enumerate(t_cross_sections):
        ...
        plt.plot(x_num, uh_pinn, label = 'uh PINNs', color = 'blue')
        plt.plot(x_num, uh_data, label = 'uh DNN', color = 'orange')
        plt.plot(x_num, uh_dPhy, label = 'uh HNN', color = 'green')
        plt.plot(x_num, uh_num[:, T[i]], label = 'uh numerical', color = 'red')
...
if plot == 3:
    ...
    title_array = [' h numerical', 'h DDNN', 'h PINNs','h HNN']

    uh_array = [uh_num, uh_data, uh_pinn, uh_dPhy*h_dPhy]   
    title_array_uh = [' uh numerical', 'uh DNN', 'uh PINNs', 'h HNN']
...
if plot == 4:
    ...
    plt.ylabel('Log loss')
    plt.plot(l_d.index, np.log(l_d), label = 'DNN', color = 'orange')
    plt.plot(l_p.index, np.log(l_p), label = 'PINNs', color = 'blue')
    plt.plot(l_h.index, np.log(l_h), label = 'DDPINNsNN', color = 'green')
    plt.legend()
